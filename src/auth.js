...
export async function proStatus(env, email) {
  ...
  if (hit) return hit;
  const row = await env.DB.prepare("SELECT pro, plan, telegram_chat_id, discord_user_id, discord_guild_id, discord_guild_name, discord_channel_id, discord_channel_name FROM customers WHERE email = ?").bind(email).first();
