...
export async function proStatus(env, email) {
  ...
  if (hit) return hit;
  const row = await env.DB.prepare("SELECT pro, plan, pro_until, telegram_chat_id, discord_user_id, discord_guild_id, discord_guild_name, discord_channel_id, discord_channel_name FROM customers WHERE email = ?").bind(email).first();
  const st = { pro: !!(row && row.pro), plan: (row && row.plan) || null, pro_until: (row && row.pro_until) || null, telegram: !!(row && row.telegram_chat_id),
    discord: { dm: !!(row && row.discord_user_id),
      channel: (row && row.discord_channel_id) ? { guild: row.discord_guild_name || "server", channel: row.discord_channel_name || "channel" } : null } };
